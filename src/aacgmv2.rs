...
mod tests {
    ...
    fn convert_geoc_coeffs() {
        ...
        let dt = NaiveDate::from_ymd_opt(year, month, day)
            ...
            .and_utc();
        let mut aacgm_model = Aacgmv2::new(dt).unwrap();
        let cmd = Transform::GeocentricToAACGMv2;
        let flav = Method::Coeffs;
        let (out_lat, out_lon, out_rad) = aacgm_model
            .convert(glat, glon, height, &cmd, &flav)
            .unwrap();

        assert_relative_eq!(out_lat, 47.595384745, max_relative = 1.0e-5);
        assert_relative_eq!(out_lon, 56.631679348, max_relative = 1.0e-5);
        assert_relative_eq!(out_rad, 1.1781454043, max_relative = 1.0e-5);

        let cmd = Transform::AACGMv2ToGeocentric;
        let (glat, glon, height) = aacgm_model
            .convert(
                out_lat,
                out_lon,
        ...
        assert_relative_eq!(glat, 45.439863, max_relative = 1.0e-5);
        assert_relative_eq!(glon, -23.477172, max_relative = 1.0e-5);
        assert_relative_eq!(height, 1135.0, max_relative = 1.0e-5);
    }
    ...
    fn convert_geoc_trace() {
        ...
        assert_relative_eq!(out_lat, 47.601149669, max_relative = 1.0e-5);
        assert_relative_eq!(out_lon, 56.629331323, max_relative = 1.0e-5);
        assert_relative_eq!(out_rad, 1.178145404, max_relative = 1.0e-5);
